...
Run the `LlamaCon99` scheme; `CallerIDExtension` embeds automatically,
...
iOS only lets a _Call Directory Extension_ label incoming calls with a name.
...
- **LlamaCon99** has Contacts access. It reads the address book, filters
...
UI strings live in `LlamaCon99/Localizable.xcstrings` (String Catalog),
...
(`LlamaCon99`, `CallerIDExtension`, `Shared`), so a new file dropped into
